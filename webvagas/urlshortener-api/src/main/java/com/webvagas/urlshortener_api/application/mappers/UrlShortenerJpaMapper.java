...
public interface UrlShortenerJpaMapper {
    ...
    @Mapping(target = "originalUrl", expression = "java(domain.getOriginalUrl().getValue())")
    @Mapping(target = "createdAt", expression = "java(domain.getCreatedAt().getValue())")
    ShortUrlEntity domainToEntity(ShortUrl domain);

    @Mapping(target = "shortenedUrl", expression = "java(generateShortUrl(domain.getCode().getValue(), baseUrl))")
